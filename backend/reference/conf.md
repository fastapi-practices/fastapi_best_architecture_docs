...
### `TOKEN_REQUEST_UNDERLYING_SECURITY`&#x20;
...
### `DATA_PERMISSION_MODEL_TEMPLATE_VARIABLES`&#x20;

数据规则模型可用模板变量

### `DATA_PERMISSION_COLUMN_TEMPLATE_VARIABLES`&#x20;

数据规则字段可用模板变量

### `DATA_PERMISSION_TEMPLATE_VARIABLES`&#x20;
...
### `OPERA_LOG_REDACT_KEYS`&#x20;

脱敏操作日志中的接口请求参数

### `OPERA_LOG_QUEUE_MAXSIZE`&#x20;
...
### `GRAFANA_PROMETHEUS_APP_NAME`&#x20;

Prometheus 中标识后端服务的应用名称

### `GRAFANA_CELERY_OTEL_SERVICE_NAME`&#x20;

Celery Worker 上报 OpenTelemetry 数据时使用的服务名称

### `GRAFANA_METRICS_PATH`&#x20;

Prometheus 抓取 FastAPI 指标数据的路径

### `GRAFANA_PROMETHEUS_EXEMPLAR_TRACE_ID_KEY`&#x20;
